...
<body>
	<section id="third" class="main">
		<div class="content dark style2">
			...
			<a href="refresh">返回抽取页</a>
			<p>			</p>
			<div class="button strolly" align="center">
			<h1>
		<em><center>查询结果</center></em>
	</h1>
	<br>
	<table border="1" bordercolor="#000000" width="1500px" align="center"  style="font-size:12px">
		<c:forEach var="Curd" items="${Curdlist}">
			<tr>
			  <c:if test="${Curd.str1=='col1'}">  
				<c:if test="${Curd.str1!=NULL}">
                 <td><a href="delete_colhis?str1=${Curd.str1}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str2<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str2}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str3<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str3}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str4<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str4}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str5<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str5}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str6<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str6}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str7<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str7}">删除</a></td>
               </c:if>
				<c:if test="${Curd.str8<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str8}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str9<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str9}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str10<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str10}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str11<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str11}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str12<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str12}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str13<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str13}">删除</a></td>
               </c:if>
               <c:if test="${Curd.str14<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str14}">删除</a></td>
               </c:if>
                <c:if test="${Curd.str15<=judge}">
                 <td><a href="delete_colhis?str1=${Curd.str15}">删除</a></td>
               </c:if>
				</c:if>
				...
				<c:if test="${Curd.str1!='col1'}"> 
				<td><a href="delete_hisrow?str1=${Curd.str1}">删除此行</a>
				</c:if>
				</td>
			</tr>
		</c:forEach>
